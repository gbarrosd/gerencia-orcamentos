...
def gerar_pdf_pagamento(orcamentos):
    fileName = "pagamentos-atrasados.pdf"
    dados = []
    for orcamento in orcamentos:
        dados.append(
            [
                f"N° {orcamento.numero} - Data entrega: {orcamento.dt_nota} - Tipo: {orcamento.tipo} - Secretaria: {orcamento.secretaria.nome} - Valor: R${orcamento.valor}"
            ],
        )
    if len(dados) == 0:
        dados.append(["SEM ORÇAMENTOS COM PAGAMENTO ATRASADO"])
    dados.insert(0, ["ORÇAMENTOS SEM PAGAMENTO"])
    pdf = SimpleDocTemplate(fileName, pagesize=A4)
    ...
    elems = []
    elems.append(tabela)
    pdf.title = "orçamentos sem pagamento"
    pdf.build(elems)
